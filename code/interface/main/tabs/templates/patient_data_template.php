...
<script type="text/html" id="patient-data-template">
    <div>
        ...
        <!-- ko with: patient -->
            ...
            <a class="btn btn-xs btn-link" data-bind="click: clickNewEncounter" href="#" title="<?php echo xla("New Visit");?>">
        ...
        <!-- /ko -->
		        ...
		        <span class="patientDataColumn">
		<?php //feysal add search buttons and make three columns ?>
		<div class="patientInfo">
                ...
                <?php //adapted from https://codepen.io/brandonkennedy/pen/yGjsi ?>
                    <form name="frm_search_globals">
                        <input type="text" id="anySearchBox"  class="<?php echo $any_search_class ?>" name="anySearchBox"  placeholder="<?php echo xla("Search ") ?>" autocomplete="off">
                        <button type="button" id="search_globals" class="btn btn-default fa-plus <?php echo $search_globals_class ?>" title='<?php echo xla("Add patient"); ?>' data-bind="event: {mousedown: viewPtFinder2.bind( $data, '<?php echo xla("The search field cannot be empty. Please enter a search term") ?>', '<?php echo attr($search_any_type); ?>')}">
						<button type="button" id="search_globals" class="btn btn-default fa-search <?php echo $search_globals_class ?>" title='<?php echo xla("Search for patient by entering whole or part of any demographics field information"); ?>' data-bind="event: {mousedown: viewPtFinder.bind( $data, '<?php echo xla("The search field cannot be empty. Please enter a search term") ?>', '<?php echo attr($search_any_type); ?>')}">
						</button>
                    </form>
                </div>
            </div>
